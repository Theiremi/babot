...
module.exports = class AIConv {
	...
	options() {
		let commands = new Builders.SlashCommandBuilder();
		commands.setName("chattingai");
		commands.setDescription("Toggle responding to the messages in this channel automatically");
		commands.setDescriptionLocalization('fr', "Bascule la réponse automatique aux messages de ce salon");
		commands.setDescriptionLocalization('en-US', "Toggle responding to the messages in this channel automatically");
		commands.addUserOption((option) => {
			option.setName('state');
			option.setDescription('On or off');
			option.setDescriptionLocalization('fr', 'Activé ou désactivé');
			option.setDescriptionLocalization('en-US', 'On or off');
	...
	async presenceUpdate(oldUser, newUser)
	{
		if(!newUser.equals(this.#_last_presence))
		{
			...
			if(typeof(oldUser.clientStatus) === 'object' && typeof(newUser.clientStatus) === 'object')
			{
				...
				{
					console.log('Status change for ' + newUser.user.tag);
			...
			{
				console.log('Custom state change for ' + newUser.user.tag);
				let emoji_formatted = {};
